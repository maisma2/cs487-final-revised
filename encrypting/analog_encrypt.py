...
# Translate digital message to analog message
def digital_to_analog(message, sample_rate=1000, duration=1):
    t = np.linspace(0, duration, int(sample_rate * duration))
    analog_signal = np.array([])
    # Give every character a different frequency
    for i, char in enumerate(message):
        freq = 100 + ord(char)
        signal_segment = chirp(t, f0=freq, f1=freq + 10, t1=duration, method='linear')
        # Append each signal segment to create a continuous signal
        analog_signal = np.concatenate((analog_signal, signal_segment))
